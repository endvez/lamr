...
def sortGtConstraints (x : String) (exps : List LinearExp) :
  ...
  exps.foldl (fun (acc : List LinearExp × List LinearExp × List LinearExp) exp =>
    let (noX, pos, neg) := acc
    match exp.find? x with
    | none        => (exp :: noX, pos, neg)
    | some coeff =>
      if coeff > 0 then (noX, exp :: pos, neg)
      else if coeff < 0 then (noX, pos, exp :: neg)
      else (exp :: noX, pos, neg)
  ) ([], [], [])
/--
...
def elimVarGtConstraints (x : String) (gtzeros : List LinearExp) : Option (List LinearExp) := Id.run do
  let (noX, pos, neg) := sortGtConstraints x gtzeros
  let mut result := noX
  for p in pos do
    for n in neg do
      let a := p.find! x
      let b := n.find! x
      let newConstraint := linearCombination (-b) (p.erase x) a (n.erase x)
      if newConstraint.isEmpty then
        return none 
      else
        result := newConstraint :: result
  some result
