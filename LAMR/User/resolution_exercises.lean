...
partial def decide_aux (vp : VerboseProof) (cnf : List Nat) : DecisionResult :=
  if cnf.contains 0 then  -- Check if the CNF contains the empty clause
    .unsat vp  -- Return unsatisfiable with the current proof
  else match getVar? vp cnf with
    | none     => .sat []  -- No variable to decide on, CNF is trivially satisfiable
    | some var =>
      let (pos, neg, rest) := getPosNegClauseNums vp var cnf
      if pos.isEmpty && neg.isEmpty then
        .sat []  -- All clauses are satisfied without further decisions
      else
        let decisionPos := decide_aux vp (rest ++ neg)  -- Assume var is true
        let decisionNeg := decide_aux vp (rest ++ pos)  -- Assume var is false
        match (decisionPos, decisionNeg) with
          | (.sat τ, _) => .sat (τ.cons (var, true))  -- Satisfying assignment found with var true
          | (_, .sat τ) => .sat (τ.cons (var, false))  -- Satisfying assignment found with var false
          | (.unsat p1, .unsat p2) =>
            let combinedProof := p1 ++ p2  -- Combine proofs for unsatisfiability
            .unsat combinedProof  -- Return combined proof
/--
