...
def allCombinations : List α → Nat → List (List α)
  | _, 0        => [[]]
  | [], _       => []
  | (x::xs), k  => (allCombinations xs (k - 1)).map (λ c => x::c) ++ allCombinations xs k

def exactlyK (L : List Lit) (k : Nat) : CnfForm :=
  let n := L.length
  let preventMoreThanKTrue := allCombinations L (k+1) |>.map (λ combo => combo.map Lit.negate)
  let forceAtLeastKTrue := allCombinations L (n-k+1)
  preventMoreThanKTrue ++ forceAtLeastKTrue
...
def negateLits (l : List Nat) : CnfForm :=
  [l.enum.map (λ (iv : Nat × Nat) => Lit.negate (mkLit iv.fst iv.snd))]
...
def solveNumberMind (puzzleLen : Nat) (guesses : List NumberMindGuess) : IO Unit := do
  ...
  for g in guesses do
    let guessCnf := exactlyK g.lits g.k
    cnf := cnf ++ guessCnf

  for i in List.range puzzleLen do
    let positionLits := List.range 10 |>.map (mkLit i)
    let positionCnf := exactlyK positionLits 1
    cnf := cnf ++ positionCnf

  let (_, result) := (← callCadical cnf)
  match result with
  | SatResult.Sat τ  =>
    match decodeSolution puzzleLen τ with
    | Except.ok soln => IO.println s!"Solution: {soln}"
    | Except.error e => IO.println s!"Error: {e}"
  | SatResult.Unsat _ => IO.println "unsat."

def solveAndTestUniqueness (puzzleLen : Nat) (guesses : List NumberMindGuess) : IO Unit := do
  let mut cnf : CnfForm := []
  ...
  let (_, firstResult) := (← callCadical cnf)
  match firstResult with
  | SatResult.Sat τ  =>
    match decodeSolution puzzleLen τ with
    | Except.ok soln =>
      IO.println s!"First solution: {soln}"

      let negatedSolutionClause := negateLits (soln)
      cnf := cnf ++ negatedSolutionClause

      let (_, secondResult) := (← callCadical cnf)
      match secondResult with
      | SatResult.Sat _ => IO.println "Not unique"
      | SatResult.Unsat _ => IO.println "Unique"
    | Except.error e => IO.println s!"Error decoding solution: {e}"
  | SatResult.Unsat _ => IO.println "No soln"
...
#eval solveAndTestUniqueness 16 toughGame
